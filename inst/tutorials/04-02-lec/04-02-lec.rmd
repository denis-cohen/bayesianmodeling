---
title: 'Lecture: Choosing between pre-implemented and custom solutions'
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
- We have now illustrated how we can continuously expand the complexity of a Stan model.
- However, so far, we have dealt with standard hierarchical models that one could equally estimate using a 'canned solution'.
...
- You can use Stan (or `brms`) to relax these assumptions.
...
- You want to build a polling aggregator for parties' poll shares in Germany, 2021-2025.
- Specifically, you want to model the week-to-week evolution of parties' vote shares, incorporating the uncertainty that arises from each poll and from their joint aggregation.
- You want to get several things right:
  - Contrast suitable generative models (e.g., a Dirichlet model for the reported shares or a multinomial model for the polled respondent counts)
  - Impose a model for the latent temporal evolution of the vote shares, e.g., as random walk
  - Purge the predictions from 'pollster' effects
  - Accommodate idiosyncrasy: In January 2024, a new competitor enters the party system---its share must be deterministically zero until then, and modeled after its entry
