---
title: 'Lecture: Fundamental concepts in Bayesian inference'
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
- explain the core logic of Bayesian inference
- distinguish priors, likelihoods, and posteriors
- interpret posterior distributions and credible intervals correctly
- understand the basic idea of MCMC sampling and convergence diagnosis
...
if (eval_learnr) {
  cat("### Let's inspect this in detail, step-by-step\n")
}
```

```{r warmup-zoom-prep, eval=FALSE}
dir.create("img")

for (n in 1:len_warmup) {
  if (n == 1) {
    warmup_density <- posterior_data %>%
      dplyr::mutate_at(.vars = vars(mu, sigma2),
                       .funs = list(lag = ~ ifelse(i == 1, ., dplyr::lag(.)))) %>%
      dplyr::slice(1:n) %>%
      ggplot(aes(x = mu, y = sigma2)) +
  ...
  tmp_plot <- ggpubr::ggarrange(
    ...
    annotate_figure(top = text_grob(paste0("Warm-up sampling: ", n, " draws")))
  
  ggsave(
    filename = paste0("img/warmup", sprintf("%02d", n), ".png"),
    plot = tmp_plot,
    width = 9,
    height = 9
  )
...
```{r warmup-zoom-display, fig.align='center', out.width='576px', eval=eval_learnr}
imgs <- list.files("img", pattern = "^warmup.*png$", full.names = TRUE)
img_animated <- magick::image_read(imgs) %>%
  magick::image_animate(fps = 2) %>%
  magick::image_resize("1152x1152")
img_animated
```
