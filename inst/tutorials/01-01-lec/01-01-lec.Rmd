---
title: 'Lecture: Introduction, R refresher, and limitations of frequentist inference'
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
## Install regplane3D from GitHub if needed
if (!("regplane3D" %in% installed.packages())) {
  if (!("remotes" %in% installed.packages())) {
    install.packages("remotes", repos = "http://cran.us.r-project.org")
  }
  remotes::install_github("denis-cohen/regplane3D")
}
...
- explain the logic of frequentist inference and sampling distributions
...
```{r bvnnorm-1-print, fig.align = "center", out.width = "70%"}
...
- For this task, we will simulate $S=1,000$ samples of size $N = 50$.
...
As we can see, there is quite some variability: While many (precisely, `r sum(samples %in% 10:20)`) of our 1,000 samples produce between 10 and 20 oat-milk drinkers (with a mean of roughly `r round(mean(samples), 2)`), the numbers go as low as `r min(samples)` and as high as `r max(samples)`.
...
The first of our 1,000 samples produces the following:

- The number of oat-milk drinkers: `r samples[1]`:
...
We can see this by looking back at our 1,000 samples:
